  ...
  if (event.target.name === "name") {
    if (!validatedInputLength(fullname, 5, errorName)) {
      ...
      event.target.addEventListener("focusout", function () {
        validatedInputLength(fullname, 5, errorName);
      });
    }
    ...
    if (!validateEmailInput(email, errorEmail)) {
      ...
      event.target.addEventListener("focusout", function () {
        validateEmailInput(email, errorEmail);
      });
    }
    ...
    if (!validatedInputLength(subject, 15, errorSubject)) {
      ...
      event.target.addEventListener("focusout", function () {
        validatedInputLength(subject, 15, errorSubject);
      });
    }
    ...
    if (!validatedInputLength(message, 25, errorMessage)) {
      ...
      event.target.addEventListener("focusout", function () {
        validatedInputLength(message, 25, errorMessage);
      });
    }
  }
...
function postQuery(data, formReportingContainer) {
  fetch("https://fluffypiranha.ajbarrett.tech/wp-json/contact-form-7/v1/contact-forms/113/feedback", { method: "POST", body: data })
    .then((response) => {
    ...
    .catch((error) => console.log("error", error), createErrorMessage(formReporting));
  formReportingContainer.innerHTML = `<p class="success">Success your message has been posted</p>`;
}
